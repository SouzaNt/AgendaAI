...
<div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-3">
    <div>
        ...
        <p class="text-muted mb-0">Siga as etapas em telas individuais para selecionar unidade, categoria, horários e itens disponíveis.</p>
    </div>
...
<div id="container-wizard">
    <div class="card card-glass border-0 p-4 p-md-5">
        <!-- Indicador de Progresso (Nodes 1 a 6) -->
        <div class="wizard-progress">
            <div class="wizard-progress-bar" id="wizard-progress-bar" style="width: 0%;"></div>
            <div class="wizard-step-node active" id="node-1" onclick="goToSlide(1)" title="Passo 1: Instituição">1</div>
            <div class="wizard-step-node" id="node-2" onclick="goToSlide(2)" title="Passo 2: Categoria">2</div>
            <div class="wizard-step-node" id="node-3" onclick="goToSlide(3)" title="Passo 3: Data">3</div>
            <div class="wizard-step-node" id="node-4" onclick="goToSlide(4)" title="Passo 4: Horários">4</div>
            <div class="wizard-step-node" id="node-5" onclick="goToSlide(5)" title="Passo 5: Itens">5</div>
            <div class="wizard-step-node" id="node-6" onclick="goToSlide(6)" title="Passo 6: Confirmação">6</div>
        </div>

        <form id="form-wizard-agendamento">
            <!-- SLIDE 1: Seleção da Instituição / Unidade -->
            <div class="wizard-slide active" id="slide-1">
                <h4 class="fw-bold text-center mb-1">Passo 1: Selecione a Instituição / Unidade</h4>
                <p class="text-muted text-center mb-4">Clique na unidade do Senac em que você deseja realizar o agendamento.</p>

                <?php if ($somenteUmaInstituicao && $instituicaoUnica): ?>
                    <input type="hidden" id="wz-instituicao" value="<?= $instituicaoUnica['id'] ?>">
                    <div class="row justify-content-center mb-4">
                        <div class="col-12 col-md-6 col-lg-5">
                            <div class="selectable-card text-center py-4 selected">
                                <div class="stat-icon blue mx-auto mb-3" style="width: 64px; height: 64px; font-size: 2rem;">
                                    <i class="fa-solid fa-building-columns"></i>
                                </div>
                                <h5 class="fw-bold mb-1"><?= htmlspecialchars($instituicaoUnica['nome']) ?></h5>
                                <p class="text-muted small mb-0">Unidade Vinculada ao seu Perfil</p>
                                <span class="badge bg-primary rounded-pill mt-2">
                                    <i class="fa-solid fa-check me-1"></i> Selecionado
                                </span>
                            </div>
                        </div>
                    </div>
                <?php else: ?>
                    <input type="hidden" id="wz-instituicao" value="<?= !empty($instituicoes) ? $instituicoes[0]['id'] : '' ?>">

                    <?php if (count($instituicoes) > 6): ?>
                        <div class="row justify-content-center mb-4">
                            <div class="col-12 col-md-6">
                                <div class="input-group">
                                    <span class="input-group-text bg-transparent border-end-0"><i class="fa-solid fa-magnifying-glass text-muted"></i></span>
                                    <input type="text" class="form-control border-start-0" id="busca-inst-card" placeholder="Pesquisar unidade por nome ou cidade..." onkeyup="filtrarCardsInstituicao()">
                                </div>
                            </div>
                        </div>
                    <?php endif; ?>

                    <div class="row g-4 justify-content-center mb-4" id="grid-instituicoes-cards" style="max-height: 420px; overflow-y: auto;">
                        <?php foreach ($instituicoes as $index => $inst): ?>
                            <div class="col-12 col-md-6 col-lg-4 card-inst-col">
                                <div class="selectable-card text-center py-4 h-100 card-inst-item <?= ($index === 0) ? 'selected' : '' ?>" id="card-inst-<?= $inst['id'] ?>" onclick="selectInstituicao(<?= $inst['id'] ?>)">
                                    <div class="stat-icon blue mx-auto mb-3" style="width: 64px; height: 64px; font-size: 2rem;">
                                        <i class="fa-solid fa-building-columns"></i>
                                    </div>
                                    <h5 class="fw-bold mb-1 text-truncate" title="<?= htmlspecialchars($inst['nome']) ?>"><?= htmlspecialchars($inst['nome']) ?></h5>
                                    <p class="text-muted small mb-0">
                                        <i class="fa-solid fa-location-dot me-1 text-primary"></i>
                                        <?= htmlspecialchars($inst['municipio'] ?? 'Unidade') ?><?= !empty($inst['estado']) ? ' - ' . htmlspecialchars($inst['estado']) : '' ?>
                                    </p>
                                    <span class="badge bg-primary rounded-pill mt-2 badge-inst-check <?= ($index === 0) ? 'd-inline-block' : 'd-none' ?>">
                                        <i class="fa-solid fa-check me-1"></i> Selecionado
                                    </span>
                                </div>
                            </div>
            ...
            <!-- SLIDE 2: Escolha do Tipo de Reserva -->
            <div class="wizard-slide" id="slide-2">
                <h4 class="fw-bold text-center mb-1">Passo 2: Categoria da Reserva</h4>
                <p class="text-muted text-center mb-4">Selecione se você deseja agendar um espaço físico ou equipamentos.</p>
                ...
                <div class="row g-4 justify-content-center mb-4">
                    ...
                    <div class="col-12 col-md-5">
                        <div class="selectable-card text-center py-4" id="card-tipo-recurso" onclick="selectTipoReserva('recurso')">
                            ...
                            <p class="text-muted small mb-0">Reserva de notebooks, projetores, som, periféricos.</p>
                        </div>
                    </div>
                </div>
            ...
            <!-- SLIDE 3: Escolha da Data -->
            <div class="wizard-slide" id="slide-3">
                <h4 class="fw-bold text-center mb-1">Passo 3: Data da Reserva</h4>
            ...
            <!-- SLIDE 4: Horários de Início e Término -->
            <div class="wizard-slide" id="slide-4">
                <h4 class="fw-bold text-center mb-1">Passo 4: Horários de Início e Término</h4>
            ...
            <!-- SLIDE 5: Seleção dos Itens Disponíveis na Unidade e Horário -->
            <div class="wizard-slide" id="slide-5">
                <div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
                    <div>
                        <h4 class="fw-bold mb-0" id="txt-slide5-titulo">Passo 5: Selecione os Itens Disponíveis</h4>
                        <p class="text-muted small mb-0" id="txt-slide5-sub">Mostrando disponibilidade real para a unidade, data e horários escolhidos.</p>
                    </div>
                ...
                <div class="d-flex justify-content-between mt-4 pt-3 border-top">
                    ...
                    <button type="button" class="btn btn-primary-custom btn-lg rounded-pill px-5" onclick="nextSlide(5)">
                        Avançar <i class="fa-solid fa-arrow-right ms-2"></i>
                    </button>
                </div>
            </div>

            <!-- SLIDE 6: Finalidade & Confirmação Resumo -->
            <div class="wizard-slide" id="slide-6">
                <h4 class="fw-bold text-center mb-1">Passo 6: Finalidade & Confirmação</h4>
                <p class="text-muted text-center mb-4">Confira os dados da reserva antes de finalizar.</p>

                <div class="row g-4">
                    <div class="col-12 col-md-6">
                        <div class="mb-3" id="container-tipo-uso">
                    ...
                    <div class="col-12 col-md-6">
                        <div class="card bg-primary-subtle border-primary p-4 rounded-4 h-100 shadow-sm">
                            ...
                            <div class="mb-2"><strong>Unidade:</strong> <span id="rsm-unidade" class="fw-bold text-primary"></span></div>
                            <div class="mb-2"><strong>Categoria:</strong> <span id="rsm-tipo" class="text-uppercase fw-bold"></span></div>
                ...
                <div class="d-flex justify-content-between mt-4 pt-3 border-top">
                    <button type="button" class="btn btn-outline-secondary rounded-pill px-4" onclick="prevSlide(6)">
                        <i class="fa-solid fa-arrow-left me-2"></i> Voltar
                    </button>
...
const TODAS_INSTITUICOES = <?= json_encode($instituicoes) ?>;

let wizardState = {
    tipoReserva: 'sala', // 'sala' ou 'recurso'
    instituicaoId: $('#wz-instituicao').val() || (TODAS_INSTITUICOES[0] ? TODAS_INSTITUICOES[0].id : 1),
    selectedItemIds: [],
    selectedItemObjects: [],
    data: '',
    horaInicio: '',
...
function selectInstituicao(id) {
    wizardState.instituicaoId = id;
    $('#wz-instituicao').val(id);
    $('.card-inst-item').removeClass('selected');
    $('.badge-inst-check').addClass('d-none').removeClass('d-inline-block');

    $(`#card-inst-${id}`).addClass('selected');
    $(`#card-inst-${id} .badge-inst-check`).removeClass('d-none').addClass('d-inline-block');
}

function filtrarCardsInstituicao() {
    const term = ($('#busca-inst-card').val() || '').toLowerCase().trim();
    $('.card-inst-col').each(function() {
...
function renderGridItens() {
    ...
    const currentInstId = wizardState.instituicaoId || $('#wz-instituicao').val();

    // Atualiza o badge da instituição selecionada no cabeçalho do Passo 5
    const instObj = (TODAS_INSTITUICOES || []).find(i => String(i.id) === String(currentInstId));
    const instNome = instObj ? instObj.nome : 'Unidade Selecionada';
    $('#badge-inst-selecionada-step5').html(`<i class="fa-solid fa-building-columns me-1 text-warning"></i> Unidade: <strong>${instNome}</strong>`);

    if (wizardState.tipoReserva === 'sala') {
        $('#txt-slide5-titulo').text('Passo 5: Selecione a Sala de Aula / Auditório');
        $('#txt-slide5-sub').text(`Exibindo salas exclusivas de ${instNome} disponíveis para o horário escolhido:`);

        const salasFiltradas = TODAS_SALAS.filter(sala => {
            if (!currentInstId || String(sala.id_instituicao_vinculada) !== String(currentInstId)) return false;
        ...
        if (salasFiltradas.length === 0) {
            grid.append(`
                <div class="col-12 text-center py-5">
                    <div class="stat-icon amber mx-auto mb-3" style="width: 64px; height: 64px; font-size: 2rem;">
                        <i class="fa-solid fa-building-circle-exclamation"></i>
                    </div>
                    <h5 class="fw-bold mb-1">Nenhuma sala de aula encontrada nesta unidade</h5>
                    <p class="text-muted small mb-0">Não há salas cadastradas exclusivamente para a unidade <strong>${instNome}</strong> ou compatíveis com a busca.</p>
                </div>
            `);
            return;
        }
                    ...
                    <div class="selectable-card ${!isLivre ? 'disabled' : ''} ${isSelected ? 'selected' : ''}" 
                        ...
                        <div class="d-flex align-items-center justify-content-between">
                            <div class="d-flex align-items-center gap-3">
                                <div class="stat-icon ${isLivre ? 'blue' : 'rose'}" style="width: 46px; height: 46px; font-size: 1.3rem;">
                                    <i class="fa-solid fa-door-open"></i>
                                </div>
                                <div>
                                    <h6 class="fw-bold mb-1">${sala.nome}</h6>
                                    ${isLivre 
                                        ? '<span class="badge bg-success-subtle text-success border border-success-subtle"><i class="fa-solid fa-check me-1"></i> Disponível</span>' 
                                        : `<span class="badge bg-danger-subtle text-danger border border-danger-subtle"><i class="fa-solid fa-ban me-1"></i> ${motivoIndisp}</span>`}
                                </div>
                            </div>
                            ${isSelected ? '<span class="badge bg-primary rounded-circle p-2 shadow-sm"><i class="fa-solid fa-check fs-6"></i></span>' : ''}
                        </div>
                    </div>
                </div>
            `);
        ...
        $('#txt-slide5-titulo').text('Passo 5: Selecione os Equipamentos');
        $('#txt-slide5-sub').text(`Exibindo equipamentos exclusivos de ${instNome} disponíveis para o horário escolhido:`);

        const recursosFiltrados = TODOS_RECURSOS.filter(rec => {
            if (!currentInstId || String(rec.id_instituicao_responsavel) !== String(currentInstId)) return false;
        ...
        if (recursosFiltrados.length === 0) {
            grid.append(`
                <div class="col-12 text-center py-5">
                    ...
                    <h5 class="fw-bold mb-1">Nenhum equipamento encontrado nesta unidade</h5>
                    <p class="text-muted small mb-0">Não há recursos/equipamentos cadastrados para a unidade <strong>${instNome}</strong> ou compatíveis com os filtros.</p>
                </div>
            `);
            return;
        }
            ...
            grid.append(`
                <div class="col-12 col-md-6 col-lg-4">
                    <div class="selectable-card ${!isLivre ? 'disabled' : ''} ${isSelected ? 'selected' : ''}" 
                        ...
                        <div class="d-flex align-items-center justify-content-between">
                            <div class="d-flex align-items-center gap-3">
                                <div class="stat-icon ${isLivre ? 'emerald' : 'rose'}" style="width: 46px; height: 46px; font-size: 1.3rem;">
                                    <i class="fa-solid ${isLivre ? 'fa-laptop' : 'fa-ban'}"></i>
                                </div>
                                <div>
                                    <h6 class="fw-bold mb-1">${rec.nome}</h6>
                                    <div class="d-flex align-items-center gap-1 mb-1">
                                        <span class="badge bg-secondary-subtle text-secondary border" style="font-size: 0.72rem;">${tipoNome}</span>
                                        ${rec.patrimonio ? `<span class="badge bg-light text-dark border" style="font-size: 0.72rem;">${rec.patrimonio}</span>` : ''}
                                    </div>
                                    ${isLivre 
                                        ? '<span class="badge bg-success-subtle text-success border border-success-subtle"><i class="fa-solid fa-check me-1"></i> Disponível</span>' 
                                        : `<span class="badge bg-danger-subtle text-danger border border-danger-subtle"><i class="fa-solid fa-ban me-1"></i> ${motivoIndisp}</span>`}
                            ...
                            ${isSelected ? '<span class="badge bg-success rounded-circle p-2 shadow-sm"><i class="fa-solid fa-check fs-6"></i></span>' : ''}
                        </div>
                    </div>
                </div>
            `);
...
function goToSlide(slideNum) {
    ...
    const percent = ((slideNum - 1) / 5) * 100;
...
function nextSlide(currentSlide) {
    ...
    if (currentSlide === 2) {
        if (!wizardState.tipoReserva) {
            bootbox.alert("<strong class='text-danger'>Atenção!</strong> Selecione a categoria (Sala ou Equipamento).");
            return;
        }
    }
    if (currentSlide === 3 && !$('#wz-data-selecionada').val()) {
    ...
    if (currentSlide === 5 && wizardState.selectedItemIds.length === 0) {
...
$('#form-wizard-agendamento').on('submit', function(e) {
    ...
    const payload = {
        ...
        tipo_uso: wizardState.tipoReserva === 'sala' ? 'Unidade' : $('#wz-tipo-uso').val(),
